...
export function TeamTasksPageClient({ tasks }: { tasks: TaskItem[] }) {
  const { openDrawer } = useDrawer();
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Team Tasks"
        description={`${tasks.length} tasks across the team`}
      ...
      <Tabs defaultValue="kanban">
        ...
        <TabsContent value="calendar"><MonthView events={calendarEvents} /></TabsContent>
      </Tabs>
    </div>
  );
}
